...
me"* with automatic page turning) has a 🔊 button. Narration is generated by
ElevenLabs through the guarded pipeline using `ELEVENLABS_NARRATOR_VOICE`,
cached on the page (`page.narration`, cleared when the words change), and
allowed on published library books too — it is derived audio of
already-moderated words, not an edit. When ElevenLabs isn't configured the
reader falls back to the browser's built-in speech synthesis, which also
